...
class ApplicationService:
    async def get_analytics(self, db: AsyncIOMotorDatabase) -> dict:
        ...
        recent_applications = [
            ApplicationResponse(**app) for app in applications
            if app.get("created_at", datetime.min) > thirty_days_ago
        ]
        ...
        success_metrics = {
            "offer_rate": (offers_count / total_applications) * 100,
            "rejection_rate": (rejections_count / total_applications) * 100,
            "pending_applications": status_counts.get("applied", 0),
            "total_interviews": interview_count,
            "applications_this_month": sum(1 for app in applications if app.get("created_at", datetime.min) > thirty_days_ago)
        }
    ...
    async def export_applications_to_excel(self, db, status: Optional[str] = None):
        ...
        async for app in db.applications.find(query):
            filtered_app = {
                "Position": app.get("position_title") or "",
                "Status": app.get("status") or "",
                "Application Date": app.get("application_date").strftime("%Y-%m-%d")
                if isinstance(app.get("application_date"), datetime)
                else (app.get("application_date") or ""),
                "Interview Date": app.get("interview_date").strftime("%Y-%m-%d")
                if isinstance(app.get("interview_date"), datetime)
                else (app.get("interview_date") or ""),
        ...
        if not applications:
            return StreamingResponse(
                io.BytesIO(b"No applications found."),
