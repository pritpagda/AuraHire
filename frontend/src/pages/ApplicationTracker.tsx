...
const ApplicationTrackerPage: React.FC = () => {
  ...
  const onDragEnd = (event: DragEndEvent) => {
    ...
    // Replace apps of this status in original applications array with reordered apps
  ...
  // New: Download Excel file
  const downloadExcel = async () => {
    setDownloadLoading(true);
    try {
      // If you want to filter by status, pass it as query param, e.g. status=applied
      // Here we download all statuses, no filter
  ...
  return (
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 text-gray-900 dark:text-gray-100 transition-colors duration-500 p-6 max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-6">
          {STATUSES.map((status) => {
            ...
            return (
              ...
              >
                <section className="flex flex-col rounded-2xl shadow-sm border-2 border-gray-200 dark:border-gray-700 bg-white dark:bg-gray-800">
                  <header
      ...
      {detailsApp && (
        <ApplicationDetailsModal app={detailsApp} onClose={closeDetails} />
      )}
    </div>
  );
};
