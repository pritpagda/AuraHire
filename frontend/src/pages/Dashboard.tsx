...
import { Briefcase, Calendar, Target, TrendingUp, Activity } from "lucide-react";
import type { DashboardMetrics, ApplicationStatus } from "../types/dashboard";
...
const Dashboard: React.FC = () => {
  ...
  if (loading) {
    return (
      <CenteredMessage message="Loading dashboard..." spinner />
    );
  }

  if (!metrics) {
    return (
      <CenteredMessage message="No data available" icon={<Briefcase className="w-12 h-12 opacity-50" />} />
    );
  }
...
const CenteredMessage: React.FC<{ message: string; spinner?: boolean; icon?: React.ReactNode }> = ({ message, spinner, icon }) => (
  <div className="min-h-screen flex items-center justify-center bg-gradient-to-br from-slate-50 to-blue-50 dark:from-gray-900 dark:to-gray-800">
    <div className="text-center space-y-4">
      {spinner && <div className="animate-spin h-12 w-12 rounded-full border-3 border-blue-600 border-t-transparent mx-auto" />}
...
const Stats: React.FC<{ metrics: DashboardMetrics }> = ({ metrics }) => (
...
const StatCard: React.FC<{
  ...
  gradient: string;
  bgGradient: string;
}> = ({ icon, label, value, gradient, bgGradient }) => (
  <div
...
const MainContent: React.FC<{ pieData: { name: string; value: number; color: string }[]; metrics: DashboardMetrics }> = ({
  pieData,
  metrics,
}) => (
...
const ApplicationStatusChart: React.FC<{ pieData: { name: string; value: number; color: string }[] }> = ({ pieData }) => (
  <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-6 hover:shadow-xl transition-shadow duration-300">
...
const SuccessMetrics: React.FC<{ metrics: DashboardMetrics["success_metrics"] }> = ({
  ...
  <div className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-6 space-y-4 hover:shadow-xl transition-shadow duration-300">
    <h2 className="text-xl font-semibold text-gray-900 dark:text-white mb-6">Success Metrics</h2>

    <Metric label="Offer Rate" value={`${metrics.offer_rate.toFixed(1)}%`} bg="bg-gradient-to-r from-green-50 to-emerald-50 dark:from-green-900/20 dark:to-emerald-900/20" text="text-green-700 dark:text-green-300" border="border-green-200 dark:border-green-800" />
    <Metric label="Rejection Rate" value={`${metrics.rejection_rate.toFixed(1)}%`} bg="bg-gradient-to-r from-red-50 to-rose-50 dark:from-red-900/20 dark:to-rose-900/20" text="text-red-700 dark:text-red-300" border="border-red-200 dark:border-red-800" />
    <Metric label="Pending Applications" value={metrics.pending_applications} bg="bg-gradient-to-r from-blue-50 to-sky-50 dark:from-blue-900/20 dark:to-sky-900/20" text="text-blue-700 dark:text-blue-300" border="border-blue-200 dark:border-blue-800" />
    <Metric label="Total Interviews" value={metrics.total_interviews} bg="bg-gradient-to-r from-amber-50 to-yellow-50 dark:from-amber-900/20 dark:to-yellow-900/20" text="text-amber-700 dark:text-amber-300" border="border-amber-200 dark:border-amber-800" />
  </div>
);
...
const Metric: React.FC<{
  ...
  text: string;
  border: string;
}> = ({ label, value, bg, text, border }) => (
  <div className={`flex justify-between items-center p-4 rounded-xl border ${bg} ${border} hover:shadow-sm transition-shadow duration-200`}>
...
const RecentApplications: React.FC<{ applications: DashboardMetrics["recent_applications"] }> = ({
  ...
  <section className="bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-lg border border-white/20 dark:border-gray-700/50 p-6 hover:shadow-xl transition-shadow duration-300">
    <div className="flex items-center gap-3 mb-6">
      <div className="p-2 bg-gray-100 dark:bg-gray-700 rounded-xl">
    ...
    {applications.length ? (
      <div className="space-y-3">
        {applications.map((app) => (
          ...
          >
            <div className="flex items-center gap-4">
              <div className="p-2 bg-gradient-to-br from-gray-100 to-gray-200 dark:from-gray-700 dark:to-gray-600 rounded-xl group-hover:from-blue-100 group-hover:to-blue-200 dark:group-hover:from-blue-900/30 dark:group-hover:to-blue-800/30 transition-colors">
                <Briefcase className="w-4 h-4 text-gray-600 dark:text-gray-300 group-hover:text-blue-600 dark:group-hover:text-blue-300" />
              </div>
              <div>
      ...
      <p className="text-gray-600 dark:text-gray-400 text-center">No recent applications</p>
    )}
  </section>
);
