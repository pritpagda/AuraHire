...
import { Home, FileText, Edit, ListChecks, Moon, Sun } from 'lucide-react';

const navItems = [
  { to: "/dashboard", icon: Home, label: "Dashboard" },
  { to: "/applications", icon: ListChecks, label: "Applications" },
  { to: "/resumes", icon: FileText, label: "Resumes" },
  { to: "/cover-letters", icon: Edit, label: "Cover Letter" },
...
const Sidebar: React.FC = () => {
  ...
  return (
    <aside className="flex flex-col w-64 h-screen bg-white/60 dark:bg-gray-900/60 backdrop-blur-lg border-r border-gray-200 dark:border-gray-800 shadow-2xl rounded-tr-3xl rounded-br-3xl overflow-hidden transition-all duration-300">
      ...
      <nav className="flex flex-col flex-grow p-4 space-y-2">
        {navItems.map(({ to, icon: Icon, label }) => (
          <NavLink
            ...
            to={to}
            className={({ isActive }) =>
              `flex items-center gap-3 p-3 rounded-xl transition-all duration-300 ease-in-out group
              ${isActive
                ? 'bg-gradient-to-r from-blue-100 to-blue-200 text-blue-800 dark:from-blue-700 dark:to-purple-700 dark:text-white font-semibold shadow-md'
