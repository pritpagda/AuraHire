...
const ApplicationDetailsModal: React.FC<Props> = ({ app, onClose }) => {
  ...
  return (
    <div className="fixed inset-0 z-50 bg-gray-900 bg-opacity-75 flex items-center justify-center p-4 sm:p-6 lg:p-8 animate-fade-in">
      <div className="relative bg-white dark:bg-gray-800 rounded-lg shadow-xl w-full max-w-xl max-h-[90vh] overflow-y-auto p-6 sm:p-8 animate-scale-in">
...
const DetailItem: React.FC<DetailItemProps> = ({ label, value, isPreformatted = false, children }) => (
  <div>
    <strong className="block mb-1 text-gray-900 dark:text-white">{label}:</strong>
    {isPreformatted ? (
      <p className="whitespace-pre-wrap text-sm">{value || children}</p>
    ) : (
