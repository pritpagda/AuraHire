...
import { Resume, OptimizationData, ATSData } from '../types/index';
import { File, Target, CheckCircle2, Bot, Search, AlertCircle, Zap, Sparkles } from 'lucide-react';
  ...
  return (
    <div className="bg-white dark:bg-gray-900 rounded-2xl shadow-xl border border-gray-100 dark:border-gray-800 overflow-hidden">
      <div className="p-6 text-gray-900 dark:text-white flex justify-between items-center">
        <div>
          ...
          <p className="text-gray-600 dark:text-gray-400">
            📅 {new Date(resume.created_at).toLocaleDateString('en-US', { year: 'numeric', month: 'long', day: 'numeric' })}
          </p>
        </div>
      ...
      <div className="p-6 space-y-6">
        <label htmlFor="job-description" className="flex items-center text-lg font-semibold text-gray-800 dark:text-gray-200 mb-2">
          ...
          <div className="pt-8 border-t border-gray-200 dark:border-gray-700 space-y-8">
            {optimization && (
              <section>
                ...
                <div className={`rounded-xl p-6 border-l-4 border-blue-500 bg-gray-50 dark:bg-gray-800`}>
                  <div className="flex justify-between items-center text-gray-700 dark:text-gray-300">
                    <span className="text-lg font-medium">Job Match Score</span>
                    <span className={`text-3xl font-bold ${colorClasses[getColor(optimization.match_percentage)]}`}>
                      {optimization.match_percentage.toFixed(1)}%
                    </span>
                  </div>
                </div>
                {optimization.missing_keywords.length > 0 && (
                  <div className="mt-6 p-6 bg-red-50 dark:bg-red-900 rounded-xl border border-red-200 dark:border-red-800">
                    ...
                    <div className="flex flex-wrap gap-2">
                      {optimization.missing_keywords.map((keyword, i) => (
                        <span
                          key={i}
                ...
                {optimization.skill_gaps.length > 0 && (
                  <div className="mt-6 p-6 bg-amber-50 dark:bg-amber-900 rounded-xl border border-amber-200 dark:border-amber-800">
                ...
                {optimization.suggestions.length > 0 && (
                  <div className="mt-6 p-6 bg-emerald-50 dark:bg-emerald-900 rounded-xl border border-emerald-200 dark:border-emerald-800">
            ...
            {ats && (
              <section>
                ...
                <div
                  className={`rounded-xl p-6 border-l-4 border-purple-500 bg-purple-50 dark:bg-purple-900 dark:bg-opacity-20 ${
                    colorClasses[getColor(ats.score)]
                  }`}
                >
                ...
                {ats.feedback.length > 0 && (
                  <div className="mt-6 p-6 bg-blue-50 dark:bg-blue-900 rounded-xl border border-blue-200 dark:border-blue-800">
                ...
                {ats.keyword_matches && (
                  <div className="mt-6 p-6 bg-gray-50 dark:bg-gray-800 rounded-xl border border-gray-200 dark:border-gray-700">
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
                      {ats.keyword_matches.exact?.map((keyword, i) => (
                        <div
                          key={`exact-${i}`}
                      ...
                      {ats.keyword_matches.partial?.map((keyword, i) => (
                        <div
                          key={`partial-${i}`}
                ...
                {ats.formatting_issues.length > 0 && (
                  <div className="mt-6 p-6 bg-orange-50 dark:bg-orange-900 rounded-xl border border-orange-200 dark:border-orange-800">
                ...
                {ats.recommendations.length > 0 && (
                  <div className="mt-6 p-6 bg-purple-50 dark:bg-purple-900 rounded-xl border border-purple-200 dark:border-purple-800">
